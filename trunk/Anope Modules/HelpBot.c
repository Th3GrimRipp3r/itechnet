...
int AnopeInit(int argc, char **argv)
{
    ...
    alog("HelpBot: loaded, message status [%d]", status);
...
int do_helpbothelp(User *u)
{
    ...
    if (cmd){
      if (!stricmp(cmd, "join")) {
            ...
            notice(s_HelpBot, u->nick, "HelpBot allows you to have Botserv bots join your registered");
            notice(s_HelpBot, u->nick, "channel Available commands are listed below; to use them,");
            notice(s_HelpBot, u->nick, "type /msg Helpbot command.  For more information on a");
            notice(s_HelpBot, u->nick, "specific command, type /msg helpbot HELP command. \n");
            notice(s_HelpBot, u->nick, "");
