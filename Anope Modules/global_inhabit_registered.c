/**
 * -----------------------------------------------------------------------------
 * Name    : global_inhabit_registered
 * Author  : Viper  <Viper@Anope.org>
 * Date    : 01/04/2008  (Last update: 11/10/2009)
 * Version : 1.1
 * -----------------------------------------------------------------------------
 * Tested     : Anope-1.8.2 + UnrealIRCd 3.2.6
 * Requires   : Anope-1.8.2
 * -----------------------------------------------------------------------------
 * This module will make Global join all registered channels and optionally
 ...
 * If used in combination with serv_inhabit, the Global inhabit command of serv_inhabit
 * will be disabled.
 ...
 * TODO:
 *
 *    - Find a way to allow Global to stay in empty channel without causing desyncs.
 *          (If this is at all possible... ) [Anope-Ad Hoc ONLY]
 *
 **/
...
# Main purpose is to indicate to other modules Global may be in channels.
#
#CSInhabitRegistered
...
# cs_inhabit_registered takes priority and only Global will remain in all channels.
#
 ...
 * trying to put Global in a channel).
 *
...
/**
 * If this is defined, Global will take revenge on whoever kicked it from the channel.
 **/
#define REVENGE
 
/**
 * If this is defined, Global will be given invisible mode (+i: Not shown in /WHO searches)
...
int AnopeInit(int argc, char **argv) {
	...
	alog("[\002global_inhabit_registered\002] Loading module...");
...
#ifdef SUPPORTED
	if (!moduleMinVersion(1,8,2,2364)) {
		alog("[\002global_inhabit_registered\002] Your version of Anope isn't supported. Please update to a newer release.");
		return MOD_STOP;
	}
	supported = 1;
#else
	...
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_JOIN_CHANNEL event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_PART_CHANNEL, cs_check_part);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_PART_CHANNEL event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_CHAN_EXPIRE, cs_expire);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_CHAN_EXPIRE event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_USER_LOGOFF, cs_check_logoff);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_USER_LOGOFF event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_CHAN_KICK, cs_check_kick_part);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_CHAN_KICK event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_CHAN_REGISTERED, cs_check_register);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_CHAN_REGISTERED event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_RELOAD, reload_config);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Can't hook to EVENT_RELOAD event");
		return MOD_STOP;
	}
	...
	if (moduleAddCommand(CHANSERV, c, MOD_HEAD) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Cannot hook to DROP command...");
		return MOD_STOP;
	}

	c = createCommand("FORBID", do_forbid, is_services_admin, -1, -1, -1, -1, -1);
	if (moduleAddCommand(CHANSERV, c, MOD_HEAD) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Cannot hook to FORBID command...");
		return MOD_STOP;
	}

	c = createCommand("SUSPEND", do_suspend, is_services_oper, -1, -1, -1, -1, -1);
	if (moduleAddCommand(CHANSERV, c, MOD_HEAD) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Cannot hook to SUSPEND command...");
		return MOD_STOP;
	}

	c = createCommand("ASSIGN", do_assign, NULL, -1, -1, -1, -1, -1);
	if (moduleAddCommand(BOTSERV, c, MOD_HEAD) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Cannot hook to ASSIGN command...");
		return MOD_STOP;
	}
	...
	if (moduleAddMessage(msg, MOD_TAIL) != MOD_ERR_OK) {
		alog("[\002global_inhabit_registered\002] Cannot hook to KICK message...");
		return MOD_STOP;
	}
	...
	alog("[\002global_inhabit_registered\002] Module loaded successfully...");
...
void AnopeFini(void) {
	...
	alog("[\002global_inhabit_registered\002] Unloading module...");
}
...
void cs_join_all(void) {
	...
	alog("%s has joined all channels.", s_GlobalNoticer);
}
...
void cs_part_all(void) {
	...
	for (i = 0; i < 256; i++) {
		for (ci = chanlists[i]; ci; ci = ci->next) {
			...
			anope_cmd_part(s_GlobalNoticer, ci->name, "Parting all channels..");

		}
	}

	alog("[global_inhabit_registered] %s has parted all channels.", s_GlobalNoticer);
}
...
void part_all_botserv_clients(void) {
	...
	alog("[global_inhabit_registered] All BotServ bots have been unassigned from all channels.");
}
...
int cs_check_join(int ac, char **av) {
	...
	/* Join Global after the user has been added to channel... */
	 ...
	 * If it s above, Global should already be there, if it s less, we don't join.. */
	...
	if (debug)
		alog("[global_inhabit_registered] debug: %s has joined %s", s_GlobalNoticer, ci->name);
...
int cs_check_part(int ac, char **av) {
	...
	/* Part Global before the user has been deleted from the channel...
	 ...
	 * If it s above, Global can stay there, if it s below, we shouldn't be there.. */
	if ((ci->c->usercount-1) >= BSMinUsers)
		return MOD_CONT;

	anope_cmd_part(s_GlobalNoticer, ci->name, NULL);

	if (debug)
		alog("[global_inhabit_registered] debug: %s has parted %s (Triggered by Part)", s_GlobalNoticer, ci->name);
...
int cs_check_kick_part(int ac, char **av) {
	...
	if ((ci->c->usercount-1) < BSMinUsers) {
		...
		if (debug)
			alog("[global_inhabit_registered] debug: %s has parted %s (Triggered by Kick)", s_GlobalNoticer, ci->name);
	}
...
int cs_check_logoff(int ac, char **av) {
	...
	for (uc = u->chans; uc; uc = uc->next) {
		if ((c = uc->chan)) {
			...
			anope_cmd_part(s_GlobalNoticer, c->name, NULL);

			if (debug)
				alog("[global_inhabit_registered] debug: %s has parted %s (Triggered by LogOff)", s_GlobalNoticer, c->name);
		}
	}
...
int cs_expire(int ac, char **av) {
	...
	/* There are too few users in channel for Global to be there.. */
	if (ci->c->usercount < BSMinUsers)
		return MOD_CONT;

	anope_cmd_part(s_GlobalNoticer, ci->name, "Channel has expired.");

	if (debug)
		alog("[global_inhabit_registered] debug: %s has parted %s (Channel Expired)", s_GlobalNoticer, ci->name);
...
int cs_check_register(int ac, char **av) {
	...
	if (debug)
		alog("[global_inhabit_registered] debug: %s has joined %s (Channel newly registered.)", s_GlobalNoticer, ci->name);
...
int do_drop(User * u) {
	...
	if (!(readonly && !is_services_admin(u)) && (chan) && (ci = cs_findchan(chan)) &&
		...
		if (!(ci->botflags & BS_NOBOT)) {
			if (ci->c && ci->c->usercount >= BSMinUsers) {
				/* Channel meets requirements so should have Global..
				 * Part it... */
				anope_cmd_part(s_GlobalNoticer, chan, "Channel has been dropped.");

				if (debug)
					alog("[global_inhabit_registered] debug: %s has parted %s (Channel Dropped)",
							s_GlobalNoticer, chan);
			}
		}
	}
...
int do_forbid(User * u) {
	...
	if ((chan) && (*chan == '#') && anope_valid_chan(chan)) {
		if ((ci = cs_findchan(chan))) {
			if (!((ci->flags & CI_VERBOTEN) || (ci->flags & CI_SUSPENDED) || (ci->botflags & BS_NOBOT))) {
				/* Channel is registered so may contain Global.. */
				if (ci->c && ci->c->usercount >= BSMinUsers) {
					/* Channel meets minimum requirements so should have Global
					 * Part it... */
					anope_cmd_part(s_GlobalNoticer, chan, "Channel has been forbidden");

					if (debug)
						alog("[global_inhabit_registered] debug: %s has parted %s (Channel Forbidden)",
								s_GlobalNoticer, chan);
				}
			}
		}
	...
	if ((chan) && (chan[0] == '#') && (ci = cs_findchan(chan)) && !(ci->flags & CI_VERBOTEN) ) {
		if (!((ci->flags & CI_SUSPENDED) || (ci->botflags & BS_NOBOT))) {
			...
			if (ci->c && ci->c->usercount >= BSMinUsers) {
				/* Channel meets minimum requirements so should have Global
				 * Part it... */
				anope_cmd_part(s_GlobalNoticer, chan, "Channel has been suspended");

				if (debug)
					alog("[global_inhabit_registered] debug: %s has parted %s (Channel Suspended)",
							s_GlobalNoticer, chan);
			}
		}
	}
...
int cs_check_rejoin(char *source, int ac, char **av) {
	...
	/* If Global got kicked, continue.. */
	if (stricmp(av[1], s_GlobalNoticer))
		return MOD_CONT;

	alog("%s was kicked from %s by %s", s_GlobalNoticer, ci->name, source);

	cs_do_join(ci);
/*
#ifdef REVENGE
	alog("[global_inhabit_registered] %s taking revenge on %s...", s_GlobalNoticer, source);
	kill_user(s_GlobalNoticer, source, "Is that all you've got? I win!");
#endif
*/
	return MOD_CONT;
}
...
void cs_do_join(ChannelInfo *ci) {
	if (BSSmartJoin) {
		if (ci->c && ci->c->bans && ci->c->bans->count > 0) {
			...
			snprintf(cs_mask, sizeof(cs_mask), "%s!%s", s_GlobalNoticer, ServiceUser);
			...
			for (ban = ci->c->bans->entries; ban; ban = next) {
				...
				if (entry_match_mask(ban, cs_mask, 0)) {
					anope_cmd_mode(s_GlobalNoticer, ci->name, "-b %s", ban->mask);
					if (ircdcap->tsmode)
						av[3] = ban->mask;
					else
						av[2] = ban->mask;

					do_cmode(s_GlobalNoticer, ac, av);
				}
			}
		}
	}
	anope_cmd_join(s_GlobalNoticer, ci->name, ci->c->creation_time);
...
int check_load(void) {
	...
	if (!DisableRaw || findModule("os_raw")) {
		alog("[\002global_inhabit_registered\002] RAW has NOT been disabled! (This is fatal!)");
		Enabled = 0;
		return MOD_STOP;
	}

	if (findModule("ircd_init")) {
		alog("[\002global_inhabit_registered\002] This module cannot be loaded alongside ircd_init. (This is fatal!)");
		Enabled = 0;
		return MOD_STOP;
	}

	if (findModule("cs_join")) {
		alog("[\002global_inhabit_registered\002] This module cannot be loaded alongside cs_join. (This is fatal!)");
		Enabled = 0;
		return MOD_STOP;
	}
	...
	if (supported) {
		if (findModule("bs_logchanmon")) {
			alog("[\002global_inhabit_registered\002] This module is unsupported in combination with bs_logchanmon.");
			supported = 0;
		} else if (findModule("ircd_gameserv")) {
			alog("[\002global_inhabit_registered\002] This module is unsupported in combination with ircd_gameserv.");
			supported = 0;
		}
	} else
		alog("[\002global_inhabit_registered\002] Module operating in unsupported mode!");
...
void load_config(void) {
	...
	if (Enabled) {
		if (!CSInhabitRegistered) {
			alog("[\002global_inhabit_registered\002] Disabling module... (Configuration directive missing.)");
			Enabled = 0;
			update_version();
		}
	}
	...
	if (debug)
		alog ("[global_inhabit_registered] debug: CSInhabitRegistered set to %d; OneClientPerChan set to %d",
...
int reload_config(int argc, char **argv) {
	...
	if (argc >= 1) {
		if (!stricmp(argv[0], EVENT_START)) {
			alog("[global_inhabit_registered]: Reloading configuration directives...");
			load_config();
		}
	}
